...
def test_valid_example_schema_passes():
    report = lint_schema_file(REPO_ROOT / "schemas" / "example.json")
...
def test_only_headerless_passes(tmp_path):
    p = _write(tmp_path, '{"headerless_columns": ["A", "B"]}')
    report = lint_schema_file(p)
    assert not report.has_errors(), report.render()


def test_only_column_renames_passes(tmp_path):
    p = _write(tmp_path, '{"column_renames": {"a": "GPSLatitude"}}')
    report = lint_schema_file(p)
    assert not report.has_errors(), report.render()
...
def test_headerless_not_a_list(tmp_path):
    p = _write(tmp_path, '{"headerless_columns": "not a list"}')
    report = lint_schema_file(p)
    assert report.has_errors()
    assert any("must be a list" in f.message for f in report.errors())


def test_headerless_non_string_entries(tmp_path):
    p = _write(tmp_path, '{"headerless_columns": ["A", 1, "B", null]}')
    report = lint_schema_file(p)
    assert report.has_errors()
    msgs = [f.message for f in report.errors()]
    assert any("'headerless_columns[1]'" in m for m in msgs)
    assert any("'headerless_columns[3]'" in m for m in msgs)


def test_headerless_duplicates_warn(tmp_path):
    p = _write(tmp_path, '{"headerless_columns": ["A", "B", "A"]}')
    report = lint_schema_file(p)
    assert not report.has_errors()
    assert any("Duplicate" in f.message for f in report.warnings())


def test_column_renames_not_object(tmp_path):
    p = _write(tmp_path, '{"column_renames": ["a", "b"]}')
    report = lint_schema_file(p)
    assert report.has_errors()
...
def test_rename_target_collision(tmp_path):
    p = _write(
        tmp_path,
        '{"column_renames": {"a": "GPSLatitude", "b": "GPSLatitude"}}',
    )
    report = lint_schema_file(p)
    assert report.has_errors()
...
def test_cross_check_warns_on_unknown_rename_source(tmp_path):
    p = _write(
        tmp_path,
        '{"headerless_columns": ["A"], "column_renames": {"B": "GPSLatitude"}}',
    )
    report = lint_schema_file(p)
    assert not report.has_errors()
    assert any("not in 'headerless_columns'" in f.message for f in report.warnings())
