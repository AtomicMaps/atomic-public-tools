"""Consistency between the vendored ``field_names.py`` and amtools' field groups.
...
than deriving them programmatically from ``field_names.py`` — tier assignment,
canonical-first ordering, and amtools-only extras are policy the canonical
module does not encode. These subset assertions enforce that whenever the
...
Offline: imports only :mod:`atomic_tools.vendored.field_names` and the group
...
def test_datetime_fields_subset_of_date_group():
    ...
    missing = set(field_names.DATETIME_FIELDS) - date_group
...
def test_oriented_orientation_fields_subset_of_optional_groups():
    ...
    for pitch, yaw, roll in field_names.CAM_ORIENTATION_PYR_FIELDS:
    ...
    for heading in field_names.CAM_HEADING_ONLY_FIELDS:
...
def test_focal_length_fields_subset_of_each_image_focal_group():
    focal_fields = {
        field_names.FOCAL_LENGTH_FIELD,
        field_names.FOCAL_LENGTH_35MM_FIELD,
        field_names.FOCAL_LENGTH_COMBINED_FIELD,
    }
    for image_type in (
        ...
        focal_group = _group_containing(referenced, field_names.FOCAL_LENGTH_FIELD)
...
def test_pc_capture_date_fields_subset_of_point_cloud_required():
    ...
    missing = set(field_names.PC_CAPTURE_DATE_LAS_FIELDS) - required_canonicals
